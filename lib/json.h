...
extern "C"
{
    ...
    enum json_parse_flags_e
    {
        ...
        /* allow trailing commas in objects and arrays. For example, both
           [true,] and
           {"a" : null,} would be allowed with this option on. */
        json_parse_flags_allow_trailing_comma = 0x1,

        /* allow unquoted keys for objects. For example, {a : null} would be
        ...
        /* allow that objects don't have to have comma separators between
           key/value pairs. */
        json_parse_flags_allow_no_commas = 0x10,

        /* allow c-style comments (either variants) to be ignored in the input
           JSON file. */
        ...
        /* allow simplified JSON to be parsed. Simplified JSON is an enabling of
           a set of other parsing options. */
        ...
        /* allow JSON5 to be parsed. JSON5 is an enabling of a set of other
           parsing options. */
     ...
     * value. json_extract_value performs 1 call to malloc for the entire
     * encoding.
     */
    json_weak struct json_value_s* json_extract_value(
      const struct json_value_s* value
    );

    /* Extracts a value and all the data that makes it up into a newly created
     * value. json_extract_value performs 1 call to alloc_func_ptr for the
     * entire encoding. If alloc_func_ptr is null then malloc is used. */
    json_weak struct json_value_s* json_extract_value_ex(
      const struct json_value_s* value,
      void* (*alloc_func_ptr)(void*, size_t),
      void* user_data
    );
     ...
     * The out_size parameter is optional as the utf-8 string is null
     * terminated. */
    json_weak void*
    json_write_minified(const struct json_value_s* value, size_t* out_size);

    /* Write out a pretty JSON utf-8 string. This string is encoded such that
     * the resultant JSON is pretty in that it is easily human readable. The
     * indent and newline parameters allow a user to specify what kind of
     * indentation and newline they want (two spaces / three spaces / tabs? \r,
     * \n, \r\n ?). Both indent and newline can be NULL, indent defaults to two
     * spaces ("  "), and newline defaults to linux newlines ('\n' as the
     * newline character). json_write_pretty performs 1 call to malloc for the
     * entire encoding. Return 0 if an error occurred (malformed JSON input, or
     * malloc failed). The out_size parameter is optional as the utf-8 string is
     * null terminated. */
    json_weak void* json_write_pretty(
      const struct json_value_s* value,
      const char* indent,
      const char* newline,
      size_t* out_size
     ...
     * string. */
    json_weak struct json_string_s* json_value_as_string(
      struct json_value_s* const value
    );

    /* Reinterpret a JSON value as a number. Returns null is the value was not a
     * number. */
    json_weak struct json_number_s* json_value_as_number(
      struct json_value_s* const value
    );

    /* Reinterpret a JSON value as an object. Returns null is the value was not
     * an object. */
    json_weak struct json_object_s* json_value_as_object(
      struct json_value_s* const value
    );

    /* Reinterpret a JSON value as an array. Returns null is the value was not
     * an array. */
    json_weak struct json_array_s* json_value_as_array(
      struct json_value_s* const value
    );
    ...
    struct json_object_element_s
    {
        ...
        /* the next object element (can be NULL if the last element in the
         * object). */
    ...
    struct json_array_element_s
    {
        /* the value of this element. */
        struct json_value_s* value;
        /* the next array element (can be NULL if the last element in the
         * array). */
    ...
    struct json_value_s
    {
        /* a pointer to either a json_string_s, json_number_s, json_object_s,
         * or. */
        ...
        void* payload;
        /* must be one of json_type_e. If type is json_type_true,
         * json_type_false, or.
         */
    ...
    enum json_parse_error_e
    {
        ...
        /* expected either a comma or a closing '}' or ']' to close an object
           or. */
        /* array! */
        ...
        /* the JSON input had unexpected trailing characters that weren't part
         * of the.
         */
        /* JSON value. */
...
json_weak int
json_hexadecimal_digit(const char c);
...
json_weak int
json_skip_whitespace(struct json_parse_state_s* state);
...
json_weak int
json_skip_c_style_comments(struct json_parse_state_s* state);
...
json_weak int
json_skip_all_skippables(struct json_parse_state_s* state);
...
json_weak int
json_get_value_size(struct json_parse_state_s* state, int is_global_object);

json_weak int
json_get_string_size(struct json_parse_state_s* state, size_t is_key);
        ...
        if ('\\' == src[offset]) {
            ...
            switch (src[offset]) {
                ...
                case 'u':
                     ...
                     * UTF-16 (those after U+10FFFF) are not legal Unicode
                     * values, and their UTF-8 encoding must be treated as an
                     * invalid byte sequence. */

                    if (high_surrogate != 0) {
                        /* we previously read the high half of the \uxxxx\uxxxx
                         * pair, so now we expect the low half. */
                        if (codepoint >= 0xdc00 && codepoint <= 0xdfff) { /* low
                                                                             surrogate
                                                                             range.
                                                                           */
                            data_size += 3;
                        ...
                        /* The codepoint is the first half of a "utf-16
                         * surrogate pair". so we need the other half for it to
                         * be valid: \uHHHH\uLLLL. */
                    ...
                    /* escaped codepoints after 0xffff are supported in json
                     * through utf-16 surrogate pairs: \uD83D\uDD25 for U+1F525.
                     */
...
json_weak int
is_valid_unquoted_key_char(const char c);
...
json_weak int
json_get_key_size(struct json_parse_state_s* state);
...
json_weak int
json_get_object_size(struct json_parse_state_s* state, int is_global_object);
...
json_weak int
json_get_array_size(struct json_parse_state_s* state);
...
json_weak int
json_get_number_size(struct json_parse_state_s* state);
...
json_get_number_size(struct json_parse_state_s* state)
{
    ...
    if ((json_parse_flags_allow_hexadecimal_numbers & flags_bitset) && (offset + 1 < size) && ('0' == src[offset]) && (('x' == src[offset + 1]) || ('X' == src[offset + 1]))) {
        ...
        switch (src[offset]) {
            ...
            default:
                if ((offset + 4) <= size && 't' == src[offset + 0] && 'r' == src[offset + 1] && 'u' == src[offset + 2] && 'e' == src[offset + 3]) {
                    state->offset += 4;
                    return 0;
                } else if ((offset + 5) <= size && 'f' == src[offset + 0] && 'a' == src[offset + 1] && 'l' == src[offset + 2] && 's' == src[offset + 3] && 'e' == src[offset + 4]) {
                    state->offset += 5;
                    return 0;
                    ...
                    state->offset += 4;
                    return 0;
                } else if ((json_parse_flags_allow_inf_and_nan & flags_bitset) && (offset + 3) <= size && 'N' == src[offset + 0] && 'a' == src[offset + 1] && 'N' == src[offset + 2]) {
                    return json_get_number_size(state);
                } else if ((json_parse_flags_allow_inf_and_nan & flags_bitset) && (offset + 8) <= size && 'I' == src[offset + 0] && 'n' == src[offset + 1] && 'f' == src[offset + 2] && 'i' == src[offset + 3] && 'n' == src[offset + 4] && 'i' == src[offset + 5] && 't' == src[offset + 6] && 'y' == src[offset + 7]) {
                    return json_get_number_size(state);
                }
            ...
            switch (src[offset++]) {
                ...
                case 'u': {
                    ...
                    if (!json_hexadecimal_value(&src[offset], 4, &codepoint)) {
                        return; /* this shouldn't happen as the value was
                                 * already validated.
                                 */
                    }
                    ...
                    if (codepoint <= 0x7fu) {
                        ...
                        )(0x80u | ((codepoint >> 12) & 0x3fu)); /* 10xxxxxx. */
                        data[bytes_written++] = (char
                        )(0x80u | ((codepoint >> 6) & 0x3fu)); /* 10xxxxxx. */
                        data[bytes_written++] =
                          (char)(0x80u | (codepoint & 0x3fu)); /* 10xxxxxx. */
                    } else {
                        /* we assume the value was validated and thus is within
                         * the valid range. */
                        data[bytes_written++] =
...
json_weak void
json_parse_key(struct json_parse_state_s* state, struct json_string_s* string);
...
json_weak void
json_parse_array(struct json_parse_state_s* state, struct json_array_s* array);
...
struct json_value_s*
json_parse(const void* src, size_t src_size)
{
    return json_parse_ex(
      src, src_size, json_parse_flags_default, json_null, json_null, json_null
    );
}
...
int
json_value_is_true(const struct json_value_s* const value)
{
    return value->type == json_type_true;
}

int
json_value_is_false(const struct json_value_s* const value)
{
    return value->type == json_type_false;
}

int
json_value_is_null(const struct json_value_s* const value)
{
...
json_weak int
json_write_get_number_size(const struct json_number_s* number, size_t* size);
...
json_weak int
json_write_get_string_size(const struct json_string_s* string, size_t* size);
...
json_weak char*
json_write_minified_value(const struct json_value_s* value, char* data);

json_weak char*
json_write_number(const struct json_number_s* number, char* data);
    ...
    if (number->number_size >= 2) {
        switch (number->number[1]) {
            ...
            case 'X':
                ...
                /* We need a copy of parsed number twice, so take a backup of
                 * it. */
                ...
                /* Restore parsed_number to its original value stored in the
                 * backup. */
                parsed_number = backup;

                /* Now use backup to take a copy of i, or the length of the
                 * string. */
                backup = i;
...
json_weak char*
json_write_string(const struct json_string_s* string, char* data);
...
json_weak char*
json_write_minified_array(const struct json_array_s* array, char* data);
